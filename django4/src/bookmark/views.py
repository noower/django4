...
'''
자동 임포트된 render(html 파일 보낼 대상, 보낼 html 파일- html 파일을 수정할때 사용할값): 웹 클라이언트에서 html파일을 보낼때 상용하는 함수
render 결과값으로는 html 코드 문자열의 반환물, 따라서 return 키워드에 render 함수호출로 사용자에게 html 파일을 전달할 수 있음
views.py

request : 웹클라이언트에 대한 정보( 요청정보, <form>으로 넘겨준 데이터, 로그인 정보, 세션정보, 요청방식)을 저장한 변수
...
def booklist(request):
    ...
    Bookmark.objects.get(조건) : 데이터베이스에 해당 모델클래스 객체 중 조건에 맞는 개체 한 개를 추출(객체)
    Bookmark.objects.filter(조건) : 데이터베이스에 해당  모델클래스의 객체중 조건에 맞는 개체들을 추출(리스트)
    Bookmark.objects.exclude(조건) : 데이터베이스에 해당 모델클래스의 객체 중 조건에 맞지 않는 개체들을 추출(리스트)
    '''
    #데이터베이스에 저장된 Bookmark 객체 모두추출
...
#request외에 매개변수 추가시, URL에서 데이터를 추출해 사용하는 것을 의미
