...
자동 임포트된 models 라는 파이썬파일에는 모델클래스를 정의할 때 필요한 클래스, 함수가 정의됨
...
models.XXXXField : 클래스변수에 어떤 형태의 값을 저장하도록 정의할지 객체를 생성할때 사용하는 클래스
models.CharField : 글자수 제한이 있는 문자열을 저장할때 사용하는 클래스
...
class Bookmark(models.Model):
    ...
    url = models.URLField()
    #__str__ : 객체를 화면에 출력할때 어떤 형태로 띄울지 정의된 함수 오버라이드 할 수 있음
    def __str__(self):
        return self.name
'''
프로젝트 우클릭 make migration, 'bookmark'
'''