...
def signup(request):
    ...
    #POST방식요청
        ...
        #사용자 입력이 유효한지 확인(아이디중복,비밀번호나 이메일 형식), db에 확인하는데 처리하는 방식이 있음
        ...
        if form.is_valid():        
            ...
            if form.cleaned_data['password'] == form.cleaned_data['password_check']:
                ...
                User.objects.create_user(아이디, 비밀번호, 이메일) : 매개변수에 들어온 입력을 바탕으로
                새로운 회원을 생성하는 함수
                '''
        ...
        else: #사용자입력이 올바르지 않은경우의 처리
            return render(request,'cl/signup.html',{'form':form, 'error':'형식에 맞지않는 입력을 했습니다.'})
#로그인
def signin(request):
    #GET방식 요청
    if request.method == "GET":
...
def signout(request):
    ...
    logout(request)
    #reverse('그룹이름:별칭', args=(추가매개변수의 값,))
    #그룹이름과 별칭이 일치하는 URL주소를 문자열형태로 반환해주는 함수
    #만약 추가매개변수가 있는 뷰함수인 경우, args매개변수를 통해
