...
ex)모델클래스 객체 목록, 모델클래스의 특정객체를 출력기능, 폼클래스 클래스를 활용하는 기능
'''
...
class Index(ListView):
    template_name = 'blog/index.html' #render 2번째
    model = Post
    context_object_name = 'objs'    #render 3번째
    paginate_by = 5
#글 상세보기
'''
DetailView : 데이터베이스에 특정 모델 클래스에서 한 개의 객체를 추출하는 제네릭뷰
단, 해당 뷰 클래스를 등록할 때, pk 매개변수에 id변수값을 넘겨주도록 URL등록을 해야함
...
#검색
#특정 조건을 만족하는 Post 객체를 리스트로 추출 후 화면에 추력
class PostSearch(ListView):
    ...
    #get_queryset 함수 : 특정 모델클래스에서 객체를 꺼낼때 어떤 조건으로 꺼내야하는지 설정할 수 있는 함수.
    ...
    def get_queryset(self):
        ...
        '''s=self.kwargs['search']는 오류발생가능 '''
        #'search' key로 데이터를 추출하되, 해당 key로 데이터가 존재하지 않으면
        #빈 문자열을 반환하도록 설정. 사전형 데이터에서 사용할 수 있는 get함수
        s=self.request.GET.get('search', '') 
        #추출한 데이터가 빈 문자열인지 확인
        if s:
            #빈 문자열이 아닌 경우 Post.objects.filter()함수를 사용해서 검색어에 맞는 객체들을 추출
            #filter(변수__조건 = 값) : 모델클래스 객체들이 특정 변수에 특정 조건을 만족하는지 확인해 만족하는 객체들을 리스트형태로 반환하는 함수
            #조건에 들어가는 명령어는 django라이브러리에서 지정함
            #icontains : 특정 변수에 우변에 값이 포함되어있으면 객체를 추출하도록 설정하는 조건
            #사용자가 입력한 키워드 (s 변수)를 제목에 포함하는 Post객체들을 추출
